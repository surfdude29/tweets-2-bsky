...
const TARGET_QUERY_ID_OPERATIONS = Object.keys(FALLBACK_QUERY_IDS);
export class TwitterClient {
    ...
    findTweetInInstructions(instructions, tweetId) {
        if (!instructions)
            return undefined;
    ...
    collectTextFields(value, keys, output) {
        if (!value)
            return;
        if (typeof value === 'string')
            return;
    ...
    extractNoteTweetText(result) {
        ...
        if (!note)
            return undefined;
        return this.firstText(note.text, note.richtext?.text, note.rich_text?.text, note.content?.text, note.content?.richtext?.text, note.content?.rich_text?.text);
    }
    extractTweetText(result) {
        return (this.extractArticleText(result) ?? this.extractNoteTweetText(result) ?? this.firstText(result?.legacy?.full_text));
    }
    ...
    collectTweetResultsFromEntry(entry) {
        const results = [];
    ...
    async getTweet(tweetId) {
        ...
        const tweetResult = response.data.tweetResult?.result ??
            this.findTweetInInstructions(response.data.threaded_conversation_with_injections_v2?.instructions, tweetId);
        ...
        const tryOnce = async () => {
            ...
            for (const queryId of queryIds) {
                ...
                try {
                    ...
                    return { success: true, tweets, had404 };
                }
                catch (error) {
