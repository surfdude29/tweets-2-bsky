...
function getChromeCookiesPath(profile) {
...
function matchesSafariDomain(domain) {
    if (!domain)
        return false;
...
function readSafariCString(buffer, start, end) {
...
function parseSafariCookieRecord(page, offset, jar, cookies) {
    ...
    if (!name || !value || !matchesSafariDomain(domain))
        return;
...
function parseSafariCookiePage(page, jar, cookies) {
    if (page.length < 12)
        return;
    if (!page.subarray(0, 4).equals(SAFARI_PAGE_SIGNATURE))
        return;
...
function parseSafariCookies(data, jar, cookies) {
    if (data.length < 8)
        return;
    if (data.subarray(0, 4).toString('utf8') !== 'cook')
        return;
    ...
    for (const pageSize of pageSizes) {
        if (cursor + pageSize > data.length)
            return;
 ...
 */
function decryptCookieValue(encryptedHex) {
    try {
...
export async function resolveCredentials(options) {
    ...
    const sourcesToTry = Array.isArray(cookieSource)
        ? cookieSource
        : cookieSource
            ? [cookieSource]
