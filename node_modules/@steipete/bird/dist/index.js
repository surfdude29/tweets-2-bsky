...
import { formatStatsLine, formatTweetUrlLine, labelPrefix, resolveOutputConfigFromArgv, resolveOutputConfigFromCommander, statusPrefix, } from './lib/output.js';
...
let output = resolveOutputConfigFromArgv(normalizedArgs, process.env, isTty);
...
const collectCookieSource = (value, previous = []) => {
    previous.push(parseCookieSource(value));
    return previous;
};
const p = (kind) => statusPrefix(kind, output);
const l = (kind) => labelPrefix(kind, output);
...
function readConfigFile(path) {
    ...
    try {
        ...
        console.error(colors.muted(`${p('warn')}Failed to parse config at ${path}: ${error instanceof Error ? error.message : String(error)}`));
        return {};
    }
}
function loadConfig() {
...
program.addHelpText('beforeAll', () => `${colors.banner('bird CLI')} ${colors.subtitle('— fast X CLI for tweeting, replying, and reading')}`);
...
program
    ...
    if (!query) {
        ...
        if (!handle) {
            console.error(`${p('err')}Could not determine current user (${who.error ?? 'Unknown error'}). Use --user <handle>.`);
            process.exit(1);
        }
        else {
            query = `@${handle}`;
